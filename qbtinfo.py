...
class QbtInfo:
    ...
    def __init__(
        ...
        if not QBT_AVAILABLE:
            raise ImportError(
                "qbittorrent-api not installed. Run: pip install qbittorrent-api"
            )
        ...
        self.username = (
            username if username is not None else os.getenv("QBT_USERNAME", "admin")
        )
        self.password = (
            password
            if password is not None
            else os.getenv("QBT_PASSWORD", "adminadmin")
        )
    ...
    def add_torrent(
        ...
        try:
            result = self.client.torrents_add(
                urls=url_or_magnet, save_path=save_path, category=category, tags=tags
            )

            if result == "Ok.":
                return {"status": "success", "message": f"Torrent added successfully"}
            else:
                return {
                    "status": "error",
    ...
    def remove_rss_feed(self, path: str) -> Dict[str, str]:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Failed to remove RSS item: {str(e)}",
            }
    ...
    def refresh_rss_feed(self, path: Optional[str] = None) -> Dict[str, str]:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Failed to refresh RSS feed: {str(e)}",
            }
    ...
    def add_rss_rule(self, rule_name: str, rule_def: Dict[str, Any]) -> Dict[str, str]:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Failed to create RSS rule: {str(e)}",
            }

    def update_rss_rule(
        self, rule_name: str, rule_def: Dict[str, Any]
    ) -> Dict[str, str]:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Failed to update RSS rule: {str(e)}",
            }

    def attach_rule_to_feeds(
        self, rule_name: str, feed_paths: List[str]
    ) -> Dict[str, str]:
        """
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Failed to attach rule to feeds: {str(e)}",
            }
    ...
    def delete_rss_rule(self, rule_name: str) -> Dict[str, str]:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Failed to delete RSS rule: {str(e)}",
            }

    def create_show_rss_rule(
        self,
        show_name: str,
        ...
        try:
            ...
            rule_def = {
                ...
                "affectedFeeds": feed_paths
                if feed_paths
                else [],  # Specific feeds or all feeds
                "ignoreDays": 0,
                "lastMatch": "",
                "addPaused": False,
            ...
            return {
                "status": "error",
                "message": f"Failed to create show RSS rule: {str(e)}",
            }
    ...
    def search_torrents(
        self, query: str, plugins: str = "all", category: str = "all"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            # Start search
            search_job = self.client.search_start(
                pattern=query, plugins=plugins, category=category
            )
            ...
            for i in range(max_wait):
                ...
                if i >= 7:
                    results_check = self.client.search_results(
                        search_id=search_job.id, limit=1
                    )
                    if (
                        isinstance(results_check, dict)
                        and len(results_check.get("results", [])) > 5
                    ):
                        break
        ...
        try:
            ...
            for torrent in torrents:
                ...
                if match:
                    ...
                    extracted_show = (
                        show_match.group(1).strip(". -_") if show_match else name
                    )
...
def format_torrents(torrents: List[Dict[str, Any]]) -> str:
    ...
    for t in torrents:
        ...
        lines.append(
            f"  Size: {format_bytes(t['size'])} (Downloaded: {format_bytes(t['downloaded'])})"
        )
        lines.append(
            f"  Speed: ↓ {format_speed(t['dlspeed'])} ↑ {format_speed(t['upspeed'])}"
        )
...
def cli_main():
    ...
    add_parser.add_argument("torrent_url", help="Magnet link or torrent URL")
    ...
    search_parser.add_argument(
        "--plugins", default="all", help="Plugins to use (default: all)"
    )
    search_parser.add_argument(
        "--category", default="all", help="Category filter (default: all)"
    )
    ...
    rss_list_feeds.add_argument(
        "--with-data", action="store_true", help="Include feed items/articles"
    )
    ...
    rss_remove_feed = rss_subparsers.add_parser(
        "remove-feed", help="Remove RSS feed or folder"
    )
    ...
    rss_add_show.add_argument(
        "--quality", default="1080p", help="Quality (default: 1080p)"
    )
    rss_add_show.add_argument("--category", help="Category")
    rss_add_show.add_argument("--path", help="Save path")
    rss_add_show.add_argument(
        "--feeds", help="Comma-separated feed paths to attach rule to"
    )
    ...
    parser.add_argument(
        "--url",
        help="qBittorrent URL (e.g. http://localhost:8080) or set QBT_URL env var",
    )
    parser.add_argument(
        "--username", help="qBittorrent username (or set QBT_USERNAME env var)"
    )
    parser.add_argument(
        "--password", help="qBittorrent password (or set QBT_PASSWORD env var)"
    )
    ...
    try:
        qbt = QbtInfo(url=args.url, username=args.username, password=args.password)
        ...
        if args.command == "list":
            ...
            result = qbt.add_torrent(
                args.torrent_url, save_path=args.path, category=args.category, tags=tags
            )
            print(f"{result['status'].upper()}: {result['message']}")

        elif args.command == "search":
            result = qbt.search_torrents(
                args.query, plugins=args.plugins, category=args.category
            )
            if "error" in result:
            ...
            if not result["results"]:
                ...
                for i, torrent in enumerate(result["results"][:20], 1):
                    ...
                    print(
                        f"   Size: {format_bytes(torrent['size'])} | Seeds: {torrent['seeds']} | Leeches: {torrent['leeches']}"
                    )
        ...
        try:
            if name == "qbt_get_torrents":
                ...
                if not arguments or "torrent_hash" not in arguments:
                    return [
                        types.TextContent(
                            type="text", text="Error: torrent_hash argument required"
                        )
                    ]
                ...
                if not arguments or "url_or_magnet" not in arguments:
                    return [
                        types.TextContent(
                            type="text", text="Error: url_or_magnet argument required"
                        )
                    ]
                ...
                if not arguments or "url" not in arguments:
                    return [
                        types.TextContent(
                            type="text", text="Error: url argument required"
                        )
                    ]

                result = qbt.add_rss_feed(arguments["url"], path=arguments.get("path"))
                return [
                    types.TextContent(type="text", text=json.dumps(result, indent=2))
                ]
                ...
                if not arguments or "path" not in arguments:
                    return [
                        types.TextContent(
                            type="text", text="Error: path argument required"
                        )
                    ]
                ...
                if (
                    not arguments
                    or "rule_name" not in arguments
                    or "feed_paths" not in arguments
                ):
                    return [
                        types.TextContent(
                            type="text",
                ...
                result = qbt.attach_rule_to_feeds(
                    arguments["rule_name"], arguments["feed_paths"]
                )
                return [
                    types.TextContent(type="text", text=json.dumps(result, indent=2))
                ]
                ...
                if not arguments or "show_name" not in arguments:
                    return [
                        types.TextContent(
                            type="text", text="Error: show_name argument required"
                        )
                    ]
                ...
                if not arguments or "rule_name" not in arguments:
                    return [
                        types.TextContent(
                            type="text", text="Error: rule_name argument required"
                        )
                    ]
